...
document.getElementById('menuForm').addEventListener('submit', (event) => {
    ...
    const newItem = {name, itemCategories, description};
    menuItems.push(newItem);

    // TODO: Use the spread operator to merge new categories into the set
    // allCategories.add(...itemCategories);
    itemCategories.forEach(category => allCategories.add(category));
...
const displayMenuItems = (items) => {
    ...
    menuContainer.innerHTML = items.map(({ name, itemCategories, description }) => `
        <div class="col-md-4">
            <div class="card mb-3">
                <div class="card-body">
                    <h5 class="card-title">${name}</h5>
                    <p><strong>Categories:</strong> ${itemCategories.join(', ')}</p>
                    <p>${description}</p>
                </div>
            </div>
        </div>
...
const filterByCategory = (category) => {
    const filteredItems = menuItems.filter(item => item.itemCategories.includes(category));
